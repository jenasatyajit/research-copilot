...
const components: Components = {
  ...
  h2: ({ children }) => (
    <h2 className="mt-6 mb-2.5 text-base font-semibold tracking-tight text-foreground">
      {children}
    </h2>
  ),
  h3: ({ children }) => (
  ...
  ul: ({ children }) => (
    <ul className="my-3 ml-1 flex list-none flex-col gap-1.5">{children}</ul>
  ),
  ol: ({ children }) => (
  ...
  li: ({ children }) => (
    <li className="relative pl-5 leading-relaxed before:absolute before:top-[0.65em] before:left-0 before:size-1.5 before:-translate-y-1/2 before:rounded-full before:bg-primary/60 [ol_&]:pl-1 [ol_&]:before:hidden">
      {children}
    </li>
  ),
  a: ({ href, children }) => (
